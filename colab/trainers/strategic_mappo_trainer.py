"""
Strategic MAPPO Trainer for Google Colab
Optimized for GPU training with strategic decision making using 30min data
"""
...
class StrategicActorNetwork(nn.Module):
    """Actor network for strategic MAPPO agents with enhanced architecture"""
...
class StrategicCriticNetwork(nn.Module):
    """Critic network for strategic value estimation with enhanced architecture"""
...
class StrategicMAPPOTrainer:
    """
    Strategic MAPPO Trainer for Long-term Trading Strategy
        ...
        for agent_idx in range(self.n_agents):
            ...
            for epoch in range(self.k_epochs):
                ...
                value_pred_clipped = values_tensor + torch.clamp(
                    current_values - torch.FloatTensor(values).to(self.device),
            ...
            for agent_idx in range(self.n_agents):
                ...
                if agent_idx == 0:  # Strategic agent
                    agent_features = np.array([
                        ...
                        len([r for r in list(self.regime_history)[-10:] if r == regime]) / 10.0
                    ])
                    ...
                    agent_features = np.array([
                        ...
                        np.std(list(self.volatility_history)[-10:]) if len(self.volatility_history) >= 10 else 0.0
                    ])
                    ...
                    agent_features = np.array([
                        ...
                        1.0 if regime in ['CRISIS', 'VOLATILE'] else 0.0
                    ])
                
                # Combine base features with agent-specific features
                if len(base_features) + len(agent_features) == self.state_dim:
                    state = np.concatenate([base_features, agent_features])
                else:
    ...
    def save_checkpoint(self, filepath: str):
        """Save strategic training checkpoint"""
        checkpoint = {
        ...
        logger.info(f"Strategic checkpoint saved to {filepath}")
    
    def load_checkpoint(self, filepath: str):
        """Load strategic training checkpoint"""
        ...
        self.portfolio_values = checkpoint.get('portfolio_values', [])
        self.sharpe_ratios = checkpoint.get('sharpe_ratios', [])
        self.max_drawdowns = checkpoint.get('max_drawdowns', [])
        ...
        logger.info(f"Strategic checkpoint loaded from {filepath}")
    
    def plot_strategic_progress(self, save_path: str = None):
        """Plot strategic training progress"""
        fig = plt.figure(figsize=(20, 15))
        
        # Episode rewards
        ax1 = plt.subplot(3, 3, 1)
        plt.plot(self.episode_rewards)
        plt.title('Strategic Episode Rewards')
        plt.xlabel('Episode')
        plt.ylabel('Reward')
        plt.grid(True)
        ...
        if len(self.episode_rewards) >= 20:
            ma_20 = np.convolve(self.episode_rewards, np.ones(20)/20, mode='valid')
            plt.plot(range(19, len(self.episode_rewards)), ma_20, label='MA20')
            if len(self.episode_rewards) >= 50:
                ma_50 = np.convolve(self.episode_rewards, np.ones(50)/50, mode='valid')
                plt.plot(range(49, len(self.episode_rewards)), ma_50, label='MA50')
        ...
        if self.volatility_history:
            plt.plot(list(self.volatility_history))
            plt.title('Market Volatility Tracking')
            plt.xlabel('Time')
